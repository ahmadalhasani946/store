...
@section('subject','Edit ' . $review->name)
    ...
    <div class="container" style="max-width:500px;">
        <form method="post" action="{{route('reviews.update',['review' => $review])}}">
            ...
            <div class="form-group">
                ...
                <div id="customers" name="customers"  style="margin: 10px 0;">
                    <select name='customer_id' id='customer_id' class='custom-select form-control'>
                        ...
                        <option @if(old('customer_id', $review->customer_id) == $customer->id) selected @endif value="{{ $customer->id }}">{{ $customer->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div id="products" name="products"  style="margin: 10px 0;">
                    <select name='product_id' id='product_id' class='custom-select form-control'>
                        ...
                        <option @if(old('product_id', $review->product_id) == $product->id) selected @endif value="{{ $product->id }}">{{ $product->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control @error('comment') error @enderror" rows="5" id="comment" name="comment" placeholder="Enter The Comment Here">{{ old('comment', $review->comment) }}</textarea>
            </div>
            @error('comment')
            ...
            <div class="form-group" style="margin:auto;">
                ...
                <div class="starrating" style="direction:rtl;float:left;">
                    @for($i = 5; $i > 0; $i--)
                        <input @if(old('stars', $review->stars) == $i) checked @endif type="radio" id="star{{$i}}" name="stars" value="{{$i}}" /><label for="star{{$i}}" title="{{$i}} star"></label>
                    @endfor
                </div>
            </div><br><br>
            @error('stars')
